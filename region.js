...
function cropYieldGraph(data, baseLayout, config) {
...
function tempRangeGraph(data, baseLayout, config) {
...
function rainfallYieldGraph(baseLayout, config) {
    ...
    Plotly.newPlot('rainYieldScatter', [{
        x: sampleData.rainfall.map(val => parseFloat(val)),
        y: sampleData.yield.map(val => parseFloat(val)),
        mode: 'markers',
        type: 'scatter',
        marker: {
            size: 5,
...
function soilTypeGraph(baseLayout, config) {
    ...
    Plotly.newPlot('soilPieChart', [{
        labels: Object.keys(soilData),
        values: Object.values(soilData),
        type: 'pie',
        marker: {
...
function renderCharts(data) {
    ...
    cropYieldGraph(data, baseLayout, config);
